...
export function QuotaLabFallback({ items }: Readonly<QuotaLabFallbackProps>) {
  ...
  return (
    <section
      className="quota-lab-fallback"
      aria-label="Active account quota fallback"
    >
      <p className="quota-lab-fallback__notice" role="status">
        WebGL is unavailable. Showing the active account CSS tube.
      </p>
      <article
      ...
      >
        <div
          ...
          style={quotaLevelStyle(item)}
          aria-hidden="true"
        >
          <span className="quota-lab-fallback__tube-cap quota-lab-fallback__tube-cap--top" />
          <span className="quota-lab-fallback__tube-fill quota-lab-fallback__tube-fill--five-hour" />
          <span className="quota-lab-fallback__tube-fill quota-lab-fallback__tube-fill--weekly" />
        ...
        <div className="quota-lab-fallback__content">
          <header className="quota-lab-fallback__header">
            <div className="quota-lab-fallback__identity-copy">
          ...
          <p className="quota-lab-fallback__updated">{item.updatedLabel}</p>
          {item.errorMessage ? (
...
function quotaLevelStyle(item: LabAccountItem): QuotaMeterStyle {
  const fiveHourFill = Math.max(
    0,
    Math.min(1, (item.fiveHour.percent ?? 0) / 100),
  );
  const weeklyFill = Math.max(
    0,
    Math.min(1, (item.weekly.percent ?? 0) / 100),
  );

  return {
    '--quota-level': `${Math.round(fiveHourFill * 86)}px`,
    '--quota-weekly-level': `${Math.round(weeklyFill * 36)}px`,
  };
}
