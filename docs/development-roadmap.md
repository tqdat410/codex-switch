...
Implementation is materially complete for the planned v1 surface. Local validation is green. Remaining work is mostly manual acceptance, packaging confidence, and publish readiness.
...
| 3. TUI Picker and Quota Capture | In Progress | Watcher and picker are implemented; live Codex-session validation remains open. |
| 4. Dashboard Core | In Progress | Home/API/switch flows exist; standalone warning and live UI smoke remain open. |
...
- history/quota ingestion helpers
...
- The project depends on Codex’s local file shapes not changing drastically.
