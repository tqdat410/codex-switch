...
- `packages/dashboard`: Next.js app, full-account command deck home, local API routes, charts, add/history pages
...
- `@codex-switch/dashboard`: reads the shared SQLite state read-only, renders the home command deck with HTML account controls plus one decorative client-only Three/R3F ambient canvas, and exposes local mutation routes that shell out to the CLI.
